...
class AttParseNet(pl.LightningModule):
    ...
    def training_epoch_end(self, outputs):
        self.log("Training Epoch Accuracy", self.train_accuracy.compute(), logger=True)
        self.log("Training Precision", self.train_precision.compute(), logger=True)
        self.log("Training Recall", self.train_recall.compute(), logger=True)
        self.log("Training F1", self.train_f1.compute(), logger=True)
    ...
    def validation_epoch_end(self, outputs):
        ...
        self.log("Validation Precision", self.valid_precision.compute(), logger=True)
        self.log("Validation Recall", self.valid_recall.compute(), logger=True)
        self.log("Validation F1", self.valid_f1.compute(), logger=True)
